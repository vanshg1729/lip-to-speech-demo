...
let startup = async () => {
    ...
    try {
        ...
        console.log('Got the stream out')
        video.play()
    } catch (error) {
...
let toggleBoundingBox = () => {
    let button = document.getElementById('bbox-button')
    if (boundingBox) {
        boundingBox = 0
        // button.textContent = 'Start Bounding Boxes'
    } else {
        boundingBox = 1
        // button.textContent = 'Stop Bounding Boxes'
    }
}
...
window.addEventListener(
    'load',
    async () => {
        await startVideoDisplay()
        let bboxButton = document.getElementById('bbox-button')
        bboxButton.addEventListener('click', toggleBoundingBox)
    },
    false
)
